...
const cleanPerformanceData = (techPerformanceData, qcPerformanceData, typeSet) => {
    ...
    for (let i = 0; i < techPerformanceData[0].length; i++) {
        techPerformanceMap.set(techPerformanceData[0][i].trim().toLowerCase(), parseFloat(techPerformanceData[1][i]));
        qcPerformanceMap.set(qcPerformanceData[0][i].trim().toLowerCase(), parseFloat(qcPerformanceData[1][i]));

        typeSet.delete(techPerformanceData[0][i].toLowerCase());
        typeSetDuplication.delete(techPerformanceData[0][i].toLowerCase());
    }
